class Container {
  ...
  constructor(
    ...
    lang = {
      ...
      addpost: "Add a comic to this storyline",
      norows: "There are no posts to display"
    }
  ) {
    let stateDefaults = {
      heading: "",
...
class StorylineContainer extends Container {
  ...
  buildContainer() {
    ...
    if(this.state.storyline !== 0){
        ...
        let pageButtons = this.addPageButtons();
        rowContainer.append(pageButtons);
    }
    //If this is the top level, don't display any pages.
    else{
        rowContainer = container.find(".row-container-pages").first();
        rowContainer.remove();
    }
  ...
  addStorylineButtons() {
    return $(`<div class="cc-btn-row dark-bg" style="padding:15px 0 0"><button class="cc-btn light-bg addstoryline" onclick="window.location.href='${this.state.root}${this.state.ccroot}modules/${this.state.moduleSlug}/add-storyline/${this.state.storyline}'">${this.lang.addstoryline}</button>
  <button class="cc-btn blue-bg" onclick="window.location.href='${this.state.root}${this.state.ccroot}modules/${this.state.moduleSlug}/rearrange-storylines/${this.state.storyline}'">Rearrange these storylines</button></div>`);
  }

  addPageButtons() {
    return $(`<div class="cc-btn-row dark-bg" style="padding:15px 0 0"><button class="cc-btn light-bg addstoryline" onclick="window.location.href='${this.state.root}${this.state.ccroot}modules/${this.state.moduleSlug}/add-post/${this.state.storyline}'">${this.lang.addpost}</button></div>`);
  }
